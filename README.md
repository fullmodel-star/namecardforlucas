# 智慧名片系統 UI/UX 與 PWA (漸進式網頁應用) 部署驗證報告

本報告新增了針對 **PWA (Progressive Web App)** 的部署與安裝驗證說明，以利您順利將系統當成手機 App 進行安裝與離線使用。

---

## 🛠 優化與 PWA 支援亮點

### 1. PWA (漸進式網頁應用) 支援 (App 化)
為了實現您的需求，並以最輕量的方式轉化為手機 App 運作，我們已建立完整的 PWA 規格：
- **`manifest.json`**：設定了 App 的名稱（智慧名片管理系統）、圖示、啟動背景色與全螢幕獨立運行模式。
- **`sw.js` (Service Worker) 離線核心**：
  - 新建了 [sw.js](file:///C:/Users/c0787/Desktop/名片管理/sw.js) 服務線程檔案。
  - 實作了 **Network-First (網路優先)** 快取防線策略：當有網路時優先下載最新版面並更新快取；當手機在斷網或收訊不佳的地方時，會自動調出本機快取的 `index.html` 與相關字型、圖示資源，**確保 App 隨時隨地可開啟用**。
- **Service Worker 註冊機制**：
  - 在 `index.html` 啟動進入點加入了註冊語句，確保瀏覽器成功偵測到 PWA 規範。

---

## 📱 手機 App 安裝與部署指南

要把本專案變成手機 App 執行，請照著以下步驟操作：

### 第一步：上傳網頁 (二選一，推薦免費部署)
PWA 規定網頁必須在具有 `https://` 的加密安全網域下才能觸發安裝。
1. **方式 A (Vercel，最推薦新手)**：
   - 到 [Vercel 官網](https://vercel.com/) 免費註冊一個帳號。
   - 安裝 Vercel CLI 或將桌面上的「名片管理」資料夾拖曳上傳至 Vercel，即可獲得一個免費的 `https://xxxx.vercel.app` 專屬網址。
2. **方式 B (GitHub Pages)**：
   - 建立一個 GitHub 儲存庫 (Repository)，將專案上傳，並在設定中開啟 GitHub Pages，即可取得 `https://username.github.io/repository-name` 免費 HTTPS 網址。

### 第二步：在手機上「安裝」為 App
* **iPhone (iOS) 系統**：
  1. 使用手機 **Safari 瀏覽器** 開啟您部署的網址。
  2. 點擊瀏覽器底部的 **「分享」** 按鈕（向上箭頭的方塊）。
  3. 滑動並選擇 **「加入主畫面」** (Add to Home Screen)。
  4. 點擊「新增」。您的手機桌面就會多出名片系統的 App 圖示！
* **Android (安卓) 系統**：
  1. 使用手機 **Chrome 瀏覽器** 開啟您的網址。
  2. 點擊右上角「三個點」選單，或直接在底部彈出的提示中點擊 **「安裝應用程式」** (Install App) 或 **「新增至主畫面」**。
  3. 確認安裝後，App 就會以原生樣式出現在您的手機應用程式列表與桌面上！

---

## 🏪 Google Play 上架進度（更新：2026-06-13）

> **目前狀態：尚未上架。Google Play 開發者帳號「還沒有申請過」。**

採 **TWA（Trusted Web Activity）** 路線——Android App 是個指向線上版網站（`fullmodel-star.github.io`）的薄殼，內容即時從網站載入，故網站更新後 **App 不需重新打包**（除非更動 App 名稱／套件 id／圖示／啟動網址）。

| 步驟 | 狀態 |
|---|---|
| ① 前置（manifest、icons 192/512/maskable、`privacy.html`、`sw.js` 離線）| ✅ 完成 |
| ② PWABuilder 產生套件（`名片管理.aab`、`.apk`、`signing.keystore`、`signing-key-info.txt`；套件名 `io.github.fullmodel_star.twa`）| ✅ 完成（桌面「名片(G)」夾）|
| `assetlinks.json` 部署到網站 `.well-known/` 並驗證（App 無網址列）| ✅ 完成 |
| ③ **註冊 Google Play 開發者帳號（一次性 US$25）** | ⬜ **尚未申請** |
| ④ Play Console 上傳 `.aab` ＋ 商店素材（截圖、功能圖 1024×500、512 圖示、隱私頁網址、說明、內容分級、資料安全表單）| ⬜ 待辦 |

**上架前須確認**：
- **資料安全表單（Data Safety）**：本 App 會用 Google 登入、把名片照片送 Google Gemini 做 OCR、可同步至 Google 通訊錄；需如實申報，並在隱私頁補述「名片照片會傳送至 Google Gemini 進行文字辨識」。
- **權限**：相機（掃名片）、NFC、讀取通訊錄（從手機匯入）需對應說明。
- **Target API level**：Google 每年（約 8 月後）調高新上架門檻；`.aab` 若擱置過久才上，可能需用 PWABuilder 重新產生以符合當時的 targetSdk。
- **keystore 備份**：`signing.keystore` 與密碼／alias 務必另存（弄丟＝永遠無法更新 App）。
- **金鑰乾淨**：Gemini 金鑰為使用者自填、存於本機，無硬編碼密鑰（對審查為加分項）。
- 上架前建議用**真手機實機跑一次「雲端匯入＋辨識」**，確認計數在真實 Gemini 回應下也正確。

---

## 🔧 Bug 修正記錄

### 2026-06-13: 多角色 UX 體檢後的全面修正（資料安全／離線／無障礙／效率）
依「6 種使用者角色 × 對抗式驗證」找出的問題，進行一輪全面修正：

* **資料安全（最高優先）**
  * **「完整備份」名實相符**：`exportFullJsonBackup` / `importFullJsonBackup` 改為含版本號的物件格式，一併打包並還原**個人名片、自訂群組、自訂標籤**（過去只備了 IndexedDB 名片，換機還原後這三類會靜默消失）。相容舊版陣列格式。
  * **重複偵測新增「合併保留一張」**：自動補齊缺漏欄位、聯集標籤、保留照片後刪除多餘者，避免「只能擇一刪」造成資料遺失。
* **離線與隱私**
  * QR Code 改為**內嵌 `qrcode-generator` (MIT) 於本機產生**，取代原本將姓名／電話／Email 送往第三方 `api.qrserver.com` 的做法 → **離線可用、聯絡資料不外傳**，並修正圓角裁切定位點安靜區的問題。`sw.js` 快取版本升至 `v15`。
* **新手上手**
  * 預設落點由空白「個人名片」改為「**總覽**」；頁首品牌與標語常駐、新增**深／淺色切換鈕**（原 `toggleTheme` 為無入口的死碼）。
  * 新增**首次使用歡迎卡**；AI 金鑰提示統一且可一鍵跳設定；批次辨識**開跑前先檢查金鑰**；拖放區文案改為手機導向「點此拍照或從相簿選取」。
* **效率（重度使用者）**
  * 名片清單**顯示公司**、新增**群組篩選**、支援**多選＋批次匯出/改群組/刪除**（吃目前勾選）；減少每張都貼「已處理」綠標的視覺雜訊。
* **無障礙／長輩**
  * 詳情刪除鈕補上文字與 `aria-label`；新增鍵盤焦點框、放大觸控目標（按鈕最小高度、社群小鈕、配色點）、底部分頁字級微調。
* **聚焦**：工具箱將 PDF 簽名／笑話／匯率／計時器等非核心功能收進「更多小工具」折疊區。
* **雲端匯入計數修正（第一輪）**：辨識進度區分「照片」與「名片」（一張照片可拆多張名片，故兩數本就不同），移除「處理中 N／完成 N」同數字的矛盾顯示；修正匯入 toast 與對話框因 `alert()` 凍結重繪造成的差一張（9 vs 10）；續抓提示標明「每批最多 100 張」。
* **雲端匯入計數修正（第二輪，3 身分模擬找出的結構性根因）**：
  * 空白照片（AI 讀不到名片）原本被靜默計為「成功」→ 改為獨立 `空白 N` 狀態、搬到雲端「辨識失敗」夾供回查。
  * 失敗(error) 照片原本永不離開佇列、跨批跨重開一直累加 → 改為辨識失敗即移出佇列，清單筆數恆等於本批 wait 數。
  * 補上**「停止辨識」按鈕**（原本 `isScanning` 無處設回 false，三個停止判斷形同虛設，掃到一半停不下來）。
  * 佇列新增**「共 N 張（待辨識／完成／失敗）」標題**；開 App 主動提示「已還原上次未辨識的 N 張」；匯入對話框標明「含先前殘留 N 張」——解決「只匯入 N 張卻顯示 /更多」的困惑。
  * 雲端項目改存正式 `fileName` 欄位（`blob.name` 在 IndexedDB 重整後會遺失，原本全顯示「雲端硬碟圖片」）。
  * 多批匯入新增**整場累計**與最後一批總結（原本只報本批、最後一批靜默結束）。完成訊息可端到端對帳：成功＋空白＋失敗＝進佇列數，再＋下載失敗＝原始張數。

### 2026-05-23: 修正 Google Drive 同步出現 textContent of null 的錯誤
* **問題原因**：
  在點擊「智慧同步導入」時，系統會調用 `showToast()` 彈出提示訊息（例如：*「正在讀取雲端未辨識照片清單...」*）。但由於 HTML 中遺漏了 `<div id="toast">` 容器，導致 `document.getElementById('toast')` 回傳 `null`，進而引發 `Cannot set properties of null (setting 'textContent')` 錯誤，造成同步流程中斷。
* **修正方案**：
  已在 [index.html](file:///C:/Users/c0787/Desktop/名片管理/index.html) 中加入 `<div id="toast" class="toast"></div>` 提示容器。現在提示與同步功能皆可正常運作。

### 2026-05-23: 優化手機版分頁欄 (Tabs) 擠壓排版
* **問題原因**：
  原先的 `.tabs` 設計沒有設定溢出滾動與防換行，在寬度較窄的手機螢幕上，五個分頁標題會被強制擠壓，導致文字直向折行（如「名、片、庫」垂直排列），視覺效果不佳且不易點擊。
* **修正方案**：
  - 將 `.tabs` 加上 `overflow-x: auto;` 與 `-webkit-overflow-scrolling: touch;`，使其在小螢幕上可以像原生 App 一樣以橫向滑動方式呈現。
  - 加上 `scrollbar-width: none;` 與 `display: none;` 隱藏瀏覽器滾動條，保持介面極簡美觀。
  - 將 `.tab` 設定 `min-width: max-content;` 與 `white-space: nowrap;`，確保文字絕不折行，保持完美的水平排版。
  - 加寬按鈕內距至 `padding: 9px 16px;`，更符合人體工學、方便手機觸控點擊。

### 2026-05-23: 「我的個人名片」支援 AI 拍照辨識自動帶入與 vCard QR Code 生成
* **新增功能**：
  - **AI 拍照/上傳辨識**：在「我的個人名片」編輯畫面中，新增了「拍照/上傳個人名片由 AI 自動填寫」功能。使用者可以直接拍照自己實體名片或上傳檔案，系統會經由 Gemini AI 進行 OCR 辨識，自動把姓名、職稱、公司、部門、電話、手機、電子信箱、地址等欄位自動填入，免去手動輸入的麻煩。
  - **vCard QR Code 動態生成**：當您的個人資料設定完成後，主畫面會自動呈現動態生成的 QR Code。當別人用手機相機掃描您的 QR Code 時，可以一鍵直接將您的聯絡資訊匯入他們手機的通訊錄中（支援 iOS 聯絡人與 Android 聯絡人）。


